...
def CAGR(DF):
    ...
    n = len(df)/(252*78) #5 minutes candle a day = 78
...
def volatility(DF):
    ...
    df = DF.copy()
    vol = df["ret"].std() * np.sqrt(252*78) #5 minutes candle a day = 78
    return vol
...
for ticker in tickers:
    ...
    for i in range(1,len(ohlc_dict[ticker])):
        if tickers_signal[ticker] == "":
            ...
            #primeira critério para comprar
        ...
        #revise stop less afiter each candle
...
#id you have 100 to alocate, you only alocate a parte of that money
